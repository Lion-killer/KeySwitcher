...
| ⚡ **Автозаміна скорочень** (`нп` → `наприклад`) | автоматично, на пробілі |
...
dotnet test  KeySwitcher.slnx --nologo     # 381 тест
```
...
| `%AppData%\KeySwitcher\debug.log` | трасування натискань і рішень (ротація: 256 КБ × 3) |
...
│   ├── Input/               # вставка тексту: Backspace × N для коротких слів, буфер обміну — для довгих
...
**381 тест: 380 проходять, 1 пропущено** (`ThresholdTuningTests.Report_MarginDistribution` — це стенд для
...
(захисні паузи), налаштування й логи. Перехоплення натискань, перемикання мови й вставку тексту
перевіряють руками — у Notepad, Chrome і VS Code.
